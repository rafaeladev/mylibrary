...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        try {
            ...
            console.error("Erreur lors de l'ajout de l'auteur à la base de données", error);
            return res.status(500).json({ error: "Erreur serveur lors de l'ajout de l'auteur" });
        }
    }
